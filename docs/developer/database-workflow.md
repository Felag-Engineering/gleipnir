...
- **`tool_input_requests`** — a durable record of a tool-initiated human-in-the-loop wait: an MCP server paused a `tools/call` with an MRTR `input_required` result (spec §6). Holds the original call (`server_id`, `tool_name`, `call_args`) so it can be retried, the opaque `request_state` the server returned (size-capped at write as defense in depth per §6.2 — enforced by the future caller, not a DB constraint), the elicitation-shaped `request_payload`, the `elicitation_kind` split from §6.1 (`permission` vs `information`), and the effective `expires_at` deadline computed per the §6.3 timeout precedence. `run_id` and `server_id` are both `ON DELETE CASCADE` — deleting the run or the server the call went to moots the wait.
  `deadline_source` (added by `0046`) records WHICH clock produced `expires_at` — the policy timeout, the server's task TTL, or a TTL the server declares for its opaque `requestState` (spec §6.3). `expires_at` alone cannot say that, and the distinction is two unrelated failures: "no human answered in time" (look at your operators or your timeout) versus "the server discarded the state needed to accept an answer" (look at the server, and consider replaying the answer). NULL means the row predates the column — unknown, not "policy". Expiry is driven by `timeout.NewToolInputScanner`, wired in `main.go` on `GLEIPNIR_FEEDBACK_SCAN_INTERVAL`: without it a host that restarts mid-wait would leave the row pending forever, since the in-process timer died with the process.

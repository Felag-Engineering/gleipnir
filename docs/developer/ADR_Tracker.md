...
| ADR-054 | `io.gleipnir/events` MCP extension — host-captured events (never model context), events/discover + events/listen, CloudEvents 1.0 envelopes (Amendment 1, 2026-08-04), at-least-once + cursor, closed-then-extensible, WG proposal path | 🟢 Implemented | v0.2.0-alpha | Replaces TriggerService gRPC; internal/mcp (`events.go`, `events_listen.go`, `cloudevents.go`), plugin-sdk/events, internal/plugin/events, internal/plugin/dedup (kept), binding pipeline (kept) — body in [mcp-realignment-spec.md](mcp-realignment-spec.md) §5, §14. Shipped in milestone #16 (#898–#904, #906; PRs #907–#919). WG proposal: [io-gleipnir-events-wg.md](../proposals/io-gleipnir-events-wg.md) (§14: implement first, then propose — the proposal exists once the conformance checklist it attaches has something to attach to). |
| ADR-055 | Tool-initiated HITL via MRTR + Tasks — permission/info split, distinct from ADR-008 gate, per-run elicitation budget + size caps + rate limit, timeout precedence, TTL answer-replay; channel delivery via `io.gleipnir/channel` extension w/ elicitation-shaped `channel/request` → Tasks task + poll-on-AuthorizeActor hint (Amendment 1, 2026-08-04) | 🟢 Implemented | v0.2.0-alpha | internal/mcp, internal/execution/agent, internal/plugin/{hitl,inapptask,decision}, audiences, ADR-031/ADR-044 (Request → durable task; RequestTerminated → tasks/cancel) — body in [mcp-realignment-spec.md](mcp-realignment-spec.md) §6. Shipped in milestone #14 (#793–#805). **Two refinements to the written spec**, both recorded in [settled-decisions.md](settled-decisions.md): (1) §6.6's "step-type additions" are *not* `model.StepType` values — they are `plugin_audit_events.event_type` values, because making them step types would make them eligible for context replay by construction; (2) §4.1's assurance gate causes an audience to *fall through*, never to fail, and `ErrNoEligibleEntry` is reachable only with the in-app fallback disabled. Poll-on-`AuthorizeActor` is still pending the host endpoint (milestone #17); until then plugin-routed resolution latency is bounded by the poll interval. Docs: [human-in-the-loop.md](../user/human-in-the-loop.md) (operators), [tool-initiated-hitl.md](tool-initiated-hitl.md) (server authors) |
| ADR-056 | Container substrate — DooD via runtime socket (rootless Podman recommended; manual-compose escape hatch), network-per-instance + subnet pool, manifest egress grants, level-triggered reconciler, OCI-image-in-Minisign bundle, per-capability health; manifest base fields aligned to MCP registry `server.json` (Amendment 1, 2026-08-04) | 🟢 Decided | v0.2.0-alpha | Replaces internal/plugin/process + generation refcounts; ADR-045 packaging updated (trust model intact) — body in [mcp-realignment-spec.md](mcp-realignment-spec.md) §7. Milestone #15 merged (#806–#820): runtime-socket client, level-triggered reconciler, per-instance networks, egress proxy, OCI bundles, per-capability health, image GC, managed MCP endpoints, and a bounded real-daemon CI lane (#820). NOT wired into `main.go` — nothing writes `plugin_containers` rows yet |
| ADR-057 | Host endpoint (amended 2026-08-04): server→host callbacks are a host-side MCP server over the per-instance internal network + instance token — gRPC/protobuf removed from the system; host-plane invariant (never in toolregistry, never grantable, internal-networks-only listener); events-vs-host boundary rule; +AuthorizeActor/SubmitIdentityProof/GetUserConfig, −WriteAuditStep/EmitEvent | 🟢 Decided | v0.2.0-alpha | internal/plugin/hostsvc (reworked to MCP server), internal/plugin/identity (kept) — body in [mcp-realignment-spec.md](mcp-realignment-spec.md) §8, full contract in [host-endpoint-contract.md](host-endpoint-contract.md). Milestone #17 decomposed 2026-08-21 (#875–#884); #875–#882 + #884 shipped 2026-08-23/24. #880 settled as a split (option C, decision record on the issue): `WriteAuditStep` removed from the live gRPC/SDK surface (#894 — Slack-routed approvals knowingly broken until the milestone #19 rewrite), `EmitEvent` removal deferred to milestone #16 (`io.gleipnir/events`). #883 (gRPC removal) stays gated until no in-tree caller remains — the v1.1 Slack/ntfy plugins and plugin-sdk serve/hostwire still ride gRPC until #19 |
...
- **ADR-053 through ADR-060 share a single body**: [mcp-realignment-spec.md](mcp-realignment-spec.md) (MCP 2026 realignment, target tag `v0.2.0-alpha`). Decided 2026-07-30; Amendment 1 accepted 2026-08-04 (spec §0: channel/config-options/identity-delivery as extensions, CloudEvents envelopes, host endpoint replaces gRPC host API, registry-aligned manifest); the §10/§11 MCP-client-compliance and schema-policy half has shipped (#733–#748, #762 — the client probes and pins a per-server protocol revision and speaks the 2026-07-28 request shaping to pinned-modern servers; the security-labelled residual in the schema-policy half is closed — #769: ADR-017's key allowlist is now enforced from the `params` block itself rather than from the narrowed schema, so it holds for root-level `oneOf`/`anyOf`/`$ref` tools too; narrowing what the agent is SHOWN still reaches only top-level properties, which the save-time warnings state) and the §6 tool-initiated HITL half has shipped too (milestone #14, #793–#805 — ADR-055 is marked Implemented above). The §5 events half has shipped too (milestone #16, #898–#904, #906; PRs #907–#919 — ADR-054 is marked Implemented above), with the same built-not-live honesty the §7 line below states for its own half: the per-instance listen supervisor (`internal/plugin/events`) is not started by `main.go`, so it goes live only once the §7 substrate cutover starts it. The §7 container substrate is BUILT BUT NOT LIVE: milestone #15 merged (#806–#820) with a bounded real-daemon CI lane (#820), but the reconciler is not started by `main.go` and nothing writes `plugin_containers` rows, so the v1.1 gRPC substrate remains the live system — see [container-substrate.md](container-substrate.md) for the same statement at package level. The §8 host endpoint half has shipped as well (milestone #17, #875–#884 + the #880 option-C removals #894/#919 — ADR-057's row above has the inventory), built-not-live on the same terms: `hostendpoint.ListenerSet` is reconciler-driven and nothing wires it until the §7 substrate goes live, with only the boot-time `AssertHostPlane` active today. Genuinely not started: §9 user scoping (#18), §12 Slack rewrite and cutover (#19, #22), and the §4.2 conformance suite (#20). Operator-facing migration notes for the shipped client half: [docs/user/mcp-protocol-migration.md](../user/mcp-protocol-migration.md). Note the phase label `v0.2.0-alpha` is a deliberate version re-baseline that post-dates the shipped `v1.0.0`/`v1.1.0` tags.

---

## ADR-061: MRTR client conformance and responder assertion
...
Gleipnir's ADR-055 tool-initiated HITL path decoded MRTR's `input_required` result as a JSON **array** of `{message, requestedSchema, _meta}` entries, correlated to the operator's answer by array position — MRTR was believed to carry no per-request id. Building the demo integration against a real MRTR server (`modelcontextprotocol/go-sdk` v1.7.0, and the Relay gateway built on it — Felag-Engineering/gleipnir-relay#646) surfaced that go-sdk's actual wire shape is a **map** keyed by a server-assigned request id (`mcp/protocol.go`, pinned by the conformance fixture `mcp/testdata/conformance/server/mrtr.txtar`):

```json
{"inputRequests": {"<id>": {"method": "elicitation/create", "params": {"mode": "form", "message": "...", "requestedSchema": {...}}}}}
```

A map from any go-sdk-based server fails to parse against Gleipnir's array decoder. Separately: Gleipnir never declared the `elicitation` client capability (`internal/mcp/registry.go` left `ClientCapabilities` at its zero value for `ResolveForPolicy`), and had no way to tell an asking server which authenticated person answered — an important gap for a server (Relay) that must refuse to let a request decide itself under a self-asserted identity (Relay's R21 guard).

### Decision

1. **Map-keyed wire shape.** `inputRequests`/`inputResponses` are objects keyed by the server-assigned request id, matching go-sdk v1.7.0. The array shape is refused outright as a structural error (`"inputRequests must be an object keyed by request id, not an array"`) — no MRTR server in this repo's test suite or production config emits it, and accepting both shapes silently would leave a real interop break undetected. Each `inputRequests` entry is `{method, params}`; only `method: "elicitation/create"` is accepted (sampling and roots/list are refused — this package has no representation for either), and only `params.mode` of `""` or `"form"` is accepted (url mode is refused, unimplemented). The elicitation-kind `_meta` hint moves from the entry's own `_meta` to `params._meta`, matching go-sdk's `ElicitParams` embedding. Decode order is deterministic (IDs sorted ascending) for this package's own internal representation; the wire itself never relies on position.

2. **`elicitation` capability declared everywhere.** `ResolveForPolicy` sets `ClientCapabilities.Elicitation = true` unconditionally for every tool it resolves, with no per-policy opt-in: a policy author cannot predict which granted tool might someday pause on `input_required`, and the capability declaration is free on any call path that never triggers it. `ResolveToolByName` (the poll trigger engine) is unaffected — poll invocations never pause a run for operator input.

3. **Responder assertion, server-side only.** The authenticated Gleipnir user who answers a tool-initiated request is asserted to the asking server in every response's own `_meta`: `inputResponses[id]._meta["io.gleipnir/responder"] = {username, user_id}`. It is filled from `auth.UserFromContext` at the HTTP handler (`tool_input_handler.go`'s `SubmitToolInput`), threaded through `RunManager.ResolveToolInput` → `agent.InputRequiredHandler.Resolve` as a new `agent.Responder{UserID, Username}` value, and stamped onto each `mcp.InputResponse.Meta` there — never read from the operator's request body, which accepts only `action` and `content`. It is sent to *every* server that asks, with no opt-in: a server that ignores it loses nothing, and a server that needs it (like Relay, refusing a self-decided request) gets it unconditionally.

4. **Every settlement path records a decision.** ADR-055 §6.6's decision-record package (`internal/plugin/decision`) had no production writer. This ADR wires one at every place a tool-initiated request reaches a terminal state, not only the common case:
   - **Answered / rejected** — `InputRequiredHandler.Route`'s answer branch, using the `Responder` delivered alongside the answer. `LinkSession` when a responder is present, `LinkNone` otherwise (e.g. the non-production `Decline` path).
   - **Timeout, this process's Route call wins the race** — the same `Route` call, via `operator_timeout.claimRequestTimeout`'s new `onWon` hook.
   - **Timeout, the background scanner wins the race** — `timeout.NewToolInputScanner`'s `WithOnTerminated` hook, wired in `main.go` to `agent.NewToolInputTimeoutDecisionHook`. This is the restart backstop: a host that restarted while a run was paused has no in-process `Route` goroutine left at all, so the scanner's own claim is the *only* settlement event that will ever exist for that row. Without this hook a restart-timed-out request settled with no decision record — a silent gap in exactly the audit trail an operator most needs after a restart.
   - **Cancelled** — `Route`'s `ctx.Done()` branch now claims the row via a new conditional `CancelToolInputRequest` query (`WHERE status='pending'`), and records `OutcomeCancelled` only when that CAS wins (`rows == 1`). The conditional guard is what keeps this safe against two races: the answer-vs-cancel race inside `Route`'s own `select` (an operator's answer can land in the buffered channel at almost the same instant the run is cancelled; Go's `select` makes no promise about which ready case it picks, but the DB write to `resolved` only happens in the answer branch, which cannot also run once the cancel branch is chosen — so there is nothing for the cancel write to clobber), and the independent-scanner race (the scanner can claim the same row as expired on its own goroutine and ticker at nearly the same moment; the conditional UPDATE arbitrates exactly one winner, and the loser records nothing).
   - **Replayed (§6.5)** — the `callToolWithInputRounds` replay branch (`prior.matches(fingerprint) && !prior.replayed`) records `OutcomeReplayedAfterTTL` with **no actor** (`decision.Record.Validate` rejects an actor on an outcome `HadActor()` reports `false` for, and no human acted at the moment of a replay). The original responder is not lost — it travels on the *wire* instead, in the `_meta` of the very same `prior.answers` the replay resends verbatim — so an auditor reading the two records side by side sees a fresh `answered` record with an actor, followed by a `replayed_after_ttl` record with none, which is the actual distinction between a human clicking twice and the host recovering from one click on their behalf.

   Each new record synthesizes `ChannelEntryID: audience.InAppEntryID` / `ChannelAssurance: authenticated` — the only channel a tool-initiated pause is answered through today (no audience routing exists for this path yet) — and, where available, the effective deadline and its source, computed once at pause time and reused across every branch.

5. **`ActorUsername` is resolved at read time, not stored.** `GET /api/v1/runs/:id/decisions`' `DecisionSummary` gained `actor_username`, looked up via `GetUser(ActorUserID)` when set; a failed lookup (e.g. a since-deleted user) is logged and leaves the field empty rather than failing the whole list. A username is mutable display text, and the record is evidence of what a Gleipnir user *ID* did — storing a username on the record would let the two drift apart.

6. **`tool_input_requests.status` gains `'cancelled'`.** SQLite cannot alter a CHECK constraint in place, so migration 0053 (`AddToolInputCancelledStatus`) rebuilds the table with the extended CHECK list, using explicit column lists on both sides of the `INSERT ... SELECT` (not `SELECT *`) so a future column addition cannot silently reorder or drop data during the rebuild. `0001_initial.sql` and `schemas/sql_schemas.sql` are hand-synced to match, per the usual sqlc-reads-0001-only rule.

7. **The prompt names the server.** `ToolInputRequestResponse` gained `server_name`, resolved from the row's `server_id` via `GetMCPServer` with a fallback to the dot prefix of `tool_name` (always nameable, since the agent called the tool under a dot-name of exactly that shape). The frontend's `ToolInputCard` renders it in the header and in the provenance line ("Asked by `{server}` mid-call...") instead of the generic "the tool server" — replacing a placeholder with the actual author of untrusted text matters when an operator is deciding how much to trust it.

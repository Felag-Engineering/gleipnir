...
| ADR-059 | LLM schema policy — canonicalize at discovery, per-provider lossy translation w/ UI notice, exact enforcement against canonical schema; ADR-017 scoping on top-level canonical properties only (oneOf-scoped rejected at save) | 🟢 Decided | v0.2.0-alpha | internal/llm (all wires), internal/mcp discovery, internal/policy validator — body in [mcp-realignment-spec.md](mcp-realignment-spec.md) §10 |
...
- **ADR-053 through ADR-060 share a single body**: [mcp-realignment-spec.md](mcp-realignment-spec.md) (MCP 2026 realignment, target tag `v0.2.0-alpha`). Decided 2026-07-30; implementation not started. Note the phase label `v0.2.0-alpha` is a deliberate version re-baseline that post-dates the shipped `v1.0.0`/`v1.1.0` tags.
...
**Header name validation:** Header names are validated with `golang.org/x/net/http/httpguts.ValidHeaderFieldName` (RFC 7230 token syntax), which rejects CR, LF, NUL, colon, and all non-token characters. A fixed reserved-name list (`Mcp-Session-Id`, `Mcp-Method`, `Mcp-Name`, `Content-Type`, `Accept`, `Content-Length`, `Host`) is additionally rejected — these headers are managed by the MCP client or the HTTP transport layer and must not be overridden. `Mcp-Method`/`Mcp-Name` added per the MCP 2026 realignment (`mcp-realignment-spec.md` §11 / ADR-053…ADR-060, issue #734); `Mcp-Session-Id` is retained through the 12-month protocol deprecation window.

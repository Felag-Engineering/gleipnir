...
- [Container substrate](container-substrate.md) — the operator's view: socket postures and what each costs in trust, why instance subnets are allocated explicitly (and the daemon `default-address-pools` tweak that keeps them from colliding), the env vars this milestone added, and how the real-daemon test suite is run and gated.
- [Container GC](container-gc.md) — what the cleanup pass reclaims and what protects a live plugin from it: the three layers guarding image removal, why a draining generation still holds its image, the conjunction that makes a subnet releasable, and the token retention window.

...
> 4. Call `todoist.create_comments` with an `items` array. Each item must contain **exactly two fields**: `task_id` (the task `id` from the trigger payload) and `content` (the formatted research). **OMIT the `project_id` field entirely** — do not include it at all, not even as an empty string, `null`, or a blank value. Including it in any form will cause Todoist to either reject the request with `Invalid ProjectID` or route the comment to the project instead of the task. The same rule applies to any other optional field you might be tempted to fill: only `task_id` and `content`, nothing else.
> 5. Call `todoist.update_tasks` with an `items` array to remove the `AI_Assist` label. Each item must contain **exactly two fields**: `task_id` (the task `id` from the trigger payload) and `labels` (the task's existing labels minus `AI_Assist`; an empty list `[]` if the task has no other labels). **OMIT every other field** — no `content`, no `description`, no `due_date`, no `priority`, no `assignee_id`, etc. Empty strings count as values and Todoist will reject them with `Invalid argument value`. Use `todoist.update_tasks` — NOT `todoist.update_labels`, which mutates a label entity by label ID and will fail with `label_id is invalid` if given a task ID.
>

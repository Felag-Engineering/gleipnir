...
Key rotation (`gleipnirctl rotate-key`) cannot help here — it requires the *old* key to decrypt the existing ciphertext. To get the instance working again, set a fresh `GLEIPNIR_ENCRYPTION_KEY` (`openssl rand -hex 32`) in `.env` and restart. The server starts normally, but every previously-stored secret is now undecryptable, so you must re-enter all provider API keys at **Admin → Models** and rotate every webhook secret from scratch.

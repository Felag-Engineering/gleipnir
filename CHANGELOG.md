...
- **Bilingual MCP request path.** A server pinned to `2026-07-28` now gets stateless `tools/list`/`tools/call` POSTs carrying `MCP-Protocol-Version`, `Mcp-Method`, and (on `tools/call`) `Mcp-Name`, with no session, no `Mcp-Session-Id`, and no 401 re-init retry; unpinned and legacy-pinned servers stay byte-for-byte unchanged (#741).
- **Typed `resultType` on `mcp.ToolResult` + consolidated MCP error-code registry.** `tools/call` results now carry a `ResultType` field, normalised from an absent/empty wire value to `"complete"` (spec §11); a non-`complete` value (`"task"`, `"input_required"`) is data, never an error — `CallTool` still returns a nil error and nothing in `internal/mcp` branches on it yet. The package's scattered JSON-RPC/MCP error-code constants are now one registry (`internal/mcp/errorcodes.go`), adding the previously-unrecorded `-32021 MissingRequiredClientCapability`. The 2026-07-28 `-32002` → `-32602` "resource not found" renumbering applies only to `resources/read`, which this client never calls, so it is a documented no-op here (#762).
- **`_meta` on every modern MCP request + capability-declaration seam.** Requests to a server pinned to `2026-07-28` now carry `_meta` with `protocolVersion`, `clientInfo`, and `clientCapabilities`, and the server's self-reported `serverInfo` is captured from `server/discover` and logged at registration/refresh. `mcp.CallTool` takes a per-request `ClientCapabilities` — threaded through `ResolvedTool` so the HITL milestone can grant `elicitation` per policy; every call path declares nothing today, and the type structurally cannot express `sampling` (ADR-026 / SEP-2577). Legacy and unpinned servers still receive byte-for-byte identical request bodies (#742).
- **Protocol version surfaced per tool source.** The MCP server DTOs now carry `protocol_version`, and each server card on the Tools page shows a badge for the pinned revision — current (`2026-07-28`), legacy, or unknown when the source has not been probed yet. Lets operators find sources still on the legacy protocol during the 12-month deprecation window (mcp-realignment-spec.md §11) (#740).
- **"Simplified for this provider" notice on the Tools page (ADR-059 / mcp-realignment-spec.md §10 step 2).** The tool DTO now carries `simplified_for`, a read-time list of every configured LLM provider whose wire will be shown a rewritten version of the tool's parameter schema; the Tools page renders an informational chip on affected tools naming the provider(s). Computed per request from the live provider registry, never persisted or cached, and only for a tool whose stored `canonical_schema` supports exact enforcement — a tool without one gets no chip rather than a fallback guess. The simplification itself runs entirely downstream of enforcement, so it changes only what the model is shown and never widens what the tool is allowed to receive (#743).
- **`ttlMs`/`cacheScope` cache hints on `tools/list` + per-server MCP client reuse.** A modern-pinned server's `tools/list` cache hint is now honored (host-clamped to a 60-second ceiling) by `Registry.RefreshTools`, so a second Discover press within the hint's TTL skips the round trip; legacy and never-probed servers are unaffected. `Registry.ResolveToolByName` also now reuses a per-server `*Client` across calls instead of building a new one every time, so the poll trigger engine stops paying a full legacy `initialize` + `notifications/initialized` handshake on every check on every tick. `CallToolResult` carries no cache hint in the 2026-07-28 schema, so poll check results are never cached — only the resolved client is reused (#748).

### Changed

- **Deterministic (sorted-by-dot-name) MCP/plugin tool ordering in the array sent to the LLM.** `buildToolDefinitions` previously iterated a Go map, so the tool array a provider received changed shape on every single run of the same policy; it now emits tools sorted by dot-name, ascending, with the synthetic `gleipnir.ask_operator` tool still appended last. Lays groundwork for provider prompt-cache stability (mcp-realignment-spec.md §11); no provider prompt caching is actually enabled anywhere in this repo yet, so there is no latency/cost change from this alone (#748).
- **Pre-dispatch argument validation against the canonical schema (ADR-059 / mcp-realignment-spec.md §10 step 3).** **MCP** tool calls are now validated against the tool's compiled canonical schema before dispatch — types, `required`, `oneOf`/`anyOf` branch selection — in addition to the existing ADR-017 key-presence gate. MCP tools with a NULL or uncompilable canonical schema fall back to the key-presence check alone. Plugin tool calls are unchanged here: they keep the ADR-017 key-presence gate only, since plugins carry no canonical schema. The two gates have deliberately different fatality: the ADR-017 key-presence gate (the operator-authored `params` boundary) still **fails the run**, exactly as before, so a violation lands in the operator attention queue; the new canonical-schema gate is **correctable** — a `tool_result` error tagged `schema_violation`, giving the agent another turn to self-correct (#744).

### Fixed

### Security

- **ADR-017 `params` scoping reports unenforceable cases instead of rejecting them (#769, option 3).** Save-time `params` validation against the stored canonical schema now emits non-blocking warnings rather than blocking the save. The earlier fail-closed behaviour blocked legitimate saves for any MCP server not rediscovered since the `canonical_schema` column landed — a fleet-wide condition an operator cannot resolve from the policy editor. **This is a deliberate trade with a real consequence:** for a tool whose schema declares no top-level `properties` (including a root-level `oneOf`/`anyOf`), `mcp.NarrowSchema` returns the schema unchanged and `mcp.ValidateCall` permits every key, so ADR-017's structural guarantee does not hold and the agent may pass any argument the tool accepts. The warning is the operator's only signal, and it names that consequence explicitly rather than saying "could not validate"; note the frontend does not currently render `data.warnings`, so it reaches API/CLI clients only. Narrowing that actually reaches inside branch keywords (#769 option 1) is deferred pending a fuller investigation and #769 remains open. Use approval gating where a tool genuinely needs constraining today.
- **Deny-all `URLLoader` on the pre-dispatch schema compiler.** The `santhosh-tekuri/jsonschema/v6` compiler used for #744's exact enforcement is constructed with a deny-all `URLLoader`, so a stored tool schema containing `"$ref":"file://…"` can no longer cause the host to read a local file at compile time (#744).
- **Static `$ref`-expansion budget on the pre-dispatch schema compiler.** `santhosh-tekuri/jsonschema/v6` has no memoization at validation time, so a compact `$defs` graph shaped as a "diamond" (each level's `allOf` branching into the same next-level pair of `$defs` entries) can cost `2^depth` leaf evaluations on every call, on a perfectly ordinary, schema-satisfying instance — an uncancellable, unkillable CPU burn that pins a core and stalls the run indefinitely. `NewArgValidator` now statically counts a schema's implied validation-path count (and its raw `$ref`/`$defs` counts) before ever compiling it, and refuses to construct the validator above a generous bound, falling back to the key-presence gate exactly like any other uncompilable canonical schema (#744).

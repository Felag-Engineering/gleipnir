...
// ── WriteAuditStep validation table ─────────────────────────────────────────

func TestWriteAuditStep(t *testing.T) {
	cases := []struct {
		name     string
		stepType string
		wantCode codes.Code
		...
		{
			name:     "feedback_response accepted",
			stepType: "feedback_response",
			wantCode: codes.OK,
		},
		{
			name:     "tool_call rejected",
			stepType: "tool_call",
			wantCode: codes.PermissionDenied,
		},
		{
			name:     "tool_result rejected",
			stepType: "tool_result",
			wantCode: codes.PermissionDenied,
		},
		{
			name:     "thought rejected",
			stepType: "thought",
			wantCode: codes.PermissionDenied,
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			h := newDefault()
			_, err := h.WriteAuditStep(context.Background(), &hostv1.WriteAuditStepRequest{
			...
			if st.Code() != tc.wantCode {
				t.Errorf("want code %v, got %v", tc.wantCode, st.Code())
			}
			// Production must mirror this exact detail string.
			if st.Message() != "unauthorized_step_type" {
				t.Errorf("want detail 'unauthorized_step_type', got %q", st.Message())
			}
		})
	}
}

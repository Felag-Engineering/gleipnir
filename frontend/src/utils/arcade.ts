...
// Returns { toolkit: '', action: name } when no dot is present.
export function splitToolkit(qualifiedName: string): { toolkit: string; action: string } {
  const dot = qualifiedName.indexOf('.')
  if (dot === -1) return { toolkit: '', action: qualifiedName }
  return {
    toolkit: qualifiedName.slice(0, dot),
    action: qualifiedName.slice(dot + 1),
  }
}
...
// Tools whose name contains no dot are skipped.

...
| `GLEIPNIR_PLUGIN_DEDUP_SWEEP_INTERVAL` | `10m` | How often the dedup sweeper evicts `plugin_event_dedup` rows older than the fixed 1-hour dedup window (#562). Only the sweep cadence is configurable; the window is fixed at 1 hour (spec §4.3). |
| `GLEIPNIR_LLM_RETRY_MAX_ATTEMPTS` | `4` | Total attempts (including the first) for a transient LLM API failure — connection errors, HTTP 408/429 (TPM/RPM rate limit), and transient 5xx (excluding permanent 501/505/506/510/511). Set to `1` to disable retry. Uniform across providers: drives the Anthropic/OpenAI SDKs' own retry count (`attempts − 1`, set at client construction) and our manual retry loop for Google + openaicompat. |
| `GLEIPNIR_LLM_RETRY_INITIAL_BACKOFF` | `1s` | Base wait for the manual retry loop (Google + openaicompat only — the Anthropic/OpenAI SDKs use their own backoff schedule). The capped target grows as `base * 2^(attempt-1)`; the actual wait is full-jitter — uniform in `[0, cap]` — to decorrelate a fleet of retriers and avoid a reconnect storm. A provider `Retry-After` hint, when present, is honored verbatim instead. |
| `GLEIPNIR_LLM_RETRY_MAX_BACKOFF` | `30s` | Ceiling for any single wait in the manual retry loop, including a provider-supplied `Retry-After` value (clamped here as a safety valve). The run context remains the hard upper bound on total wait. |
...
internal/
  ...
  execution/          — agent runtime subsystem
    ...
    run/              — run lifecycle: RunManager (goroutine tracking), RunLauncher (concurrency + launch), AgentFactory, RunsHandler (HTTP endpoints for run inspection/control), sentinel concurrency errors. `RunLauncher.LaunchWithConcurrency` is the single deep seam every trigger path goes through: it owns the `CheckConcurrency → skip / Enqueue / Launch` choreography and returns a typed `LaunchResult{Outcome, RunID}` (skip/queue are non-error outcomes; only genuine failures return an error). The four sentinels stay (`ErrConcurrencySkipActive`, `ErrConcurrencyQueueActive`, `ErrConcurrencyQueueFull`, `ErrConcurrencyUnrecognised`); raw DB errors are wrapped so callers classify them via `IsConcurrencyCheckError`/`IsEnqueueError` predicates. The six trigger handlers (webhook, manual, scheduled, poll, cron, plugin subscribed dispatcher) are thin adapters over this one method (#503).
...
- **Channel routing model (ADR-044):** Two channel operations: `Notify` (fire-and-forget, parallel fan-out, 10s deadline) and `Request` (exactly-one routing, async callback via `WriteAuditStep`). Audiences are named, ordered, admin-managed channel-entry lists referenced by policy YAML. `gleipnir.in-app` is auto-appended as lowest-priority fallback unless explicitly disabled. Six credential strategies (none, static_api_key, header_set, basic_auth, oauth2_authcode, oauth2_clientcred). Partially supersedes ADR-031 (dispatcher implementation, not the first-class-feedback principle).

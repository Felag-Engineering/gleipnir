...
| `GLEIPNIR_MCP_TIMEOUT` | `30s` | Instance-wide default timeout for MCP server calls. A server's own call timeout (set in the app on the MCP server page, `call_timeout_seconds`, 1–600s) overrides it for that server; discovery and the events stream are not affected. |
...
| `GLEIPNIR_PLUGIN_SUBNET_POOL` | `10.83.0.0/16` | Base CIDR the reconciler carves per-instance plugin networks from, one `/24` each (ADR-056, spec §7). The default holds 256 instances. Must be IPv4, `/8` to `/24`; wider than `/8` is rejected as unlikely to be intentional, narrower than `/24` leaves no room for even one per-instance allocation — both validated at startup (`config.Load`). The process also refuses to start if the pool's range overlaps one of the host's own interface *subnets* (`internal/infra/netguard.CheckPoolOverlap`, #1021 — subnet-vs-subnet via `netip.Prefix.Overlaps`, so an on-link LAN collision is caught even when no single interface address happens to sit inside the pool) — there is no override for that check. Because the guard also refuses any connection whose **client** address falls in the pool, not just the address it arrived on, a legitimate operator whose own machine sits in a range that overlaps the pool will be refused too; the fix there is choosing a non-overlapping pool, not a code change. Operators running other workloads on the same daemon should also widen the daemon's own `default-address-pools` so the two allocators do not collide — see `docs/developer/container-networking.md`. |
...
internal/
  ...
  infra/              — cross-cutting leaf packages; no internal imports
    ...
    netguard/         — strict leaf (no internal imports): net.Listener wrapper around main.go's operator API listener that refuses a connection whose kernel-assigned LocalAddr or TCP RemoteAddr falls inside GLEIPNIR_PLUGIN_SUBNET_POOL, or in fe80::/10 (#1021). RemoteAddr denies only, never allows — see the package doc for why. `WithOnRefuse` is how a caller hooks metrics into it without an internal import (main.go wires `gleipnir_operator_api_refused_total`); `Installed(ln)` lets a caller assert the guard is in the chain — the future v2 assembly (#962) must call it before enabling plugin self-attach. Full rationale, the startup pool/interface-overlap check (subnet-vs-subnet, not address-vs-pool), and composition requirements: internal/infra/netguard/listener.go.
  ...
  relaysmoke/         — build-tagged (`relaysmoke`) cross-product smoke suite against a live Relay demo fleet (issue #931); manual-only today (nightly CI is deferred until Relay publishes demo-fleet images, gleipnir-relay#451 item 5), never ci-local (every file carries the tag, so `go build/vet/test/list ./...` never see the package) — `make relaysmoke RELAY_DIR=<gleipnir-relay checkout>` to run it locally. Full contract: `docs/developer/relay-smoke.md`.
...
- `/api/v1/runs/*` — list/get runs, steps, cancel, approval, feedback; `GET|POST /:runID/tool-input` reads and answers a tool-initiated HITL pause (ADR-055). Read access matches the rest of run detail; the *write* role gate is per-row, not per-route — `permission` requests need `approver`, `information` requests need `operator` (`model.ElicitationKind.RequiredRole`), so the route middleware only excludes auditors and the handler decides. The GET may carry `prior_attempt` (spec §6.5): a server whose MRTR state expired and then re-asked a *different* question re-prompts the operator with the previous question and answer attached. An *identical* re-ask of an `information` question is replayed once against the fresh `requestState`, re-keyed onto the new request IDs, with no responder identity asserted (ADR-061). A `permission` question is **never** replayed: an identical re-ask always goes back to a human, flagged as an identical re-ask with the prior answer attached. `GET /:runID/decisions` returns the run's tool-initiated HITL **decision records** (ADR-055 §6.6, #803) — a separate endpoint from `/steps` rather than extra entries in it, because `/steps` is the trace the model is replayed and oversight evidence is not part of it (ADR-046). Read-only and readable by auditors; a combined timeline is assembled client-side.
- `/api/v1/mcp/servers/*` — MCP server registry, tool discovery; `PUT /:id` updates name/url and optional `ca_cert_pem` (absent = unchanged, "" = clear; public, read back in full with parsed subject/SHA-256/expiry) and optional `call_timeout_seconds` (1–600; absent = unchanged, 0 = clear to the GLEIPNIR_MCP_TIMEOUT default; POST accepts it too; responses carry `call_timeout_seconds` and `effective_call_timeout_seconds`) and optional `run_attribution` `{mode: off|relay|custom, on_behalf_of_header, session_ref_header, traceparent_header}` (absent = unchanged, `{mode:"off"}` = clear; custom names validated like ADR-039 headers, `[A-Za-z0-9-]` only, and must not collide with the server's auth headers or each other; values come only from run context and ride tools/call + MRTR retries, never discovery or poll — issue #943); `PUT /:id/headers/:name` and `DELETE /:id/headers/:name` manage individual auth headers (admin|operator only; see ADR-039; also 400s on a name the server's run attribution setting uses); `POST /:id/arcade/authorize` and `POST /:id/arcade/authorize/wait` pre-authorize Arcade toolkits (admin|operator only; see ADR-040)
...
- **MRTR client conformance (ADR-061):** map-keyed inputRequests/inputResponses per go-sdk (array shape refused); elicitation declared to every modern server; responder asserted server-side in response `_meta["io.gleipnir/responder"]`.

...
internal/
  ...
  infra/              — cross-cutting leaf packages; no internal imports
    ...
    headervalidate/   — HTTP header-name validation (RFC 7230 + reserved-name blocklist — see the package for the authoritative list; reserved `Mcp-*` names track the 2026-07-28 protocol client work, #734/#737). Extracted from `internal/mcp` so `internal/plugin/oauth` can validate `header_set` entries without an import cycle (#226).
...
- **Policy-level parameter scoping (ADR-017):** per-policy `params` blocks narrow the tool schema before the agent sees it — structural enforcement, not prompt-based.

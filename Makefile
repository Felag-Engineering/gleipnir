...
# Reject committed private key material (Minisign secret keys, PEM private keys).
lint-secret-keys:
	./scripts/lint-secret-keys.sh
	./scripts/lint-secret-keys-self-test.sh
...
lint: lint-go-fmt lint-plugins lint-secret-keys lint-staticcheck lint-substrate-build lint-relaysmoke-build lint-substratev2-build
...
ci-local-lint: lint-go-fmt lint-plugins lint-plugins-self-test lint-secret-keys ci-local-scope-self-test lint-staticcheck lint-substrate-build lint-relaysmoke-build lint-substratev2-build
...
# Same rationale as lint-substrate-build, for the relaysmoke suite (#931): it
# is behind the `relaysmoke` build tag, so no default lane compiles it, and a
# change to a package it imports (internal/mcp, internal/execution/run,
# internal/http/api, internal/testutil) could break it silently until the
# next nightly run. This is COMPILE-ONLY — it vets, never runs, and needs no
# Docker or Relay checkout.
lint-relaysmoke-build:
	@GOWORK=off go vet -tags relaysmoke ./internal/relaysmoke/

# The substratev2 tag set (issue #948) is the OTHER half of the one-substrate
# split: unlike substrate/relaysmoke it is not excluded from ./..., since
# internal/plugin/assembly is an ordinary importable package (only main.go's
# OWN substratev2-tagged files select which pluginSubsystem gets linked). What
# this lane catches instead is a package-main file under the wrong tag —
# go build/vet/test/staticcheck all whole-tree, mirroring CI's "Build
# (substratev2)" job exactly so a local pass means the same thing that job's
# green check does.
lint-substratev2-build:
	@GOWORK=off go build -tags substratev2 ./...
	@GOWORK=off go vet -tags substratev2 ./...
	@GOWORK=off go test -tags substratev2 .
	@GOWORK=off go run honnef.co/go/tools/cmd/staticcheck@v0.7.0 -tags substratev2 ./...

# relaysmoke drives a live, compose-started Relay demo fleet (issue #931) —
# never part of ci-local, and never run on a PR: it needs Docker, a
# gleipnir-relay checkout, and several minutes for a cold nine-Node fleet.
# See docs/developer/relay-smoke.md.
relaysmoke:
